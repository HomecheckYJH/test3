      ...
      <div v-if="filesData.length > 0" class="ui-file-grid"
        ...
        <div v-for="(file, index) in filesData" :key="index" class="ui-file-grid-item">
          <div class="btns" v-if="props.config.rotate">
            <UIButton type="white" size="sm" @click="rotateFile(index, 90)"><i class="bx bx-rotate-right"
                style="font-size:14px;"></i></UIButton>
            <UIButton type="white" size="sm" @click="rotateFile(index, 270)"><i class="bx bx-rotate-left"
                style="font-size:14px;"></i></UIButton>
          </div>
          <img style="flex:1" :src="toURL(file)" :class="[toExtension(file.type)]" />
