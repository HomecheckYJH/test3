                ...
                <li class="item-wrapper" v-for="(item) in notification_List" :key="item.id">
                    <div class="item">
                        <div class="body-wrapper"
                            :class="[item.id == current_notification_id && touch_active ? 'active' : '', item.style ? item.style : '']">
                            <div class="body" v-on:touchstart="touch(item, true, item.click)"
                                ...
                                <div v-show="item.status != 'none'" style="margin: 8px 0 8px 16px">
                                    ...
                                    <UIAnimationStatus :status="item.status"
                                        v-else-if="item.status && item.status != 'none'" />
                                </div>
                            ...
                            <div class="progress-bar" :class="[item.pending ? 'pending' : '']"
                                :style="`transition: width ${item.duration}ms linear;`"></div>
                        </div>
                    </div>
                </li>
            </transition-group>
