    ...
    <transition name="modal" duration="400">
        <div v-if="modalValue.visible" class="modal-parent">
            ...
            <div class="contents">
                ...
                <input v-if="computedPrompt() == 'Object'" style="width: 100%" :placeholder="modalValue.prompt.placeholder" v-model="inputValue"/>
                <div v-if="computedPrompt() == 'Array'">
                <input  
                class="prompt-multiple"
                v-for="(input,ip) in modalValue.prompt" :key="ip" 
                @input="promptChanged"
                v-model="inputValue_multiple[input.target]"
                style="width: 100%"
                :placeholder="input.placeholder"
                />
                </div>
                <div v-if="modalValue.selection">
                <UIButton v-for="(selection,si) in modalValue.selection" :key="si" class="prompt-multiple" type="selection" size="full">
                  {{selection.label}}
                </UIButton>
                </div>
            ...
            <div class="footer" :class="[modalValue.style ? 'transparent' : '']">
                ...
                <button 
                v-if="modalValue.confirm ? modalValue.confirm.hidden == false : true"
                v-wave="{
                    color : '#050507'
